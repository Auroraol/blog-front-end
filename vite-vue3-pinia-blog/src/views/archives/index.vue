...
import { ref } from 'vue'
// import { pageArchives, pagePublishedArticle } from '@/api/index'
...
const pageArchives = async () => {
  loading.value = true
  try {
    // const res = await pageArchives({ size: tabSize.value, current: tabCurrent.value })
    ...
    tabs.value = records
    tabPages.value = res.data.pages
    tabClick(0, tabs.value[0])
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
...
const pageArticle = async () => {
  loading.value = true
  try {
    const res = await pagePublishedArticle({ current: current.value, size: size.value, yearMonth: yearMonth.value })
    const records = res.data.records
    records.forEach(ele => {
