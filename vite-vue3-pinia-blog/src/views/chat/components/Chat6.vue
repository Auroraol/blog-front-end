...
<template>
  <div class="Chat">
    ...
    <div class="list">
      <div
        v-for="(item, index) in roomChatList[useChatStorePinia.room as number]"
        :key="index"
        class="msg"
      >
        <div
          :class="
            item.name === useChatStorePinia.chatUserInfo?.name ? 'self' : ''
          "
        >
...
import { ChatUserInfo, ChatInfo } from "/@/types/chat";
...
const addChat = (data: ChatInfo) => {
  if (roomChatList[useChatStorePinia.room as number]) {
    roomChatList[useChatStorePinia.room as number].push(data);
  } else {
    roomChatList[useChatStorePinia.room as number] = [];
    roomChatList[useChatStorePinia.room as number].push(data);
  }
};
...
const socket = io("http://localhost:9974/Chat", {
...
// 选择 | 加入房间
// watch(useChatStorePinia.room, (room) => {
//   socket.emit("joinRoom", room);

//   roomChatList[useChatStorePinia.room as number] = [];

//   getChatList(useChatStorePinia.room as number);
// });
...
const sendMsg = () => {
  ...
  // 加入房间
  socket.emit("joinRoom", useChatStorePinia.room);

  // 在这个房间中发送消息
  socket.emit("roomMsg", {
    avatar: avatarFilter(useChatStorePinia.chatUserInfo?.avatar as string),
    name: useChatStorePinia.chatUserInfo?.name as string,
